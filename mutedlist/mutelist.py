...
class MemberReasonModal(discord.ui.Modal, title="Set mute reason"):
    ...
    def __init__(self, cog: "MuteList", guild: discord.Guild, member_id: int, requester: discord.abc.User) -> None:
        super().__init__()
        self.cog = cog
        self.guild = guild
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:  # type: ignore[override]
        try:
            ...
            log.exception("Failed to save reason: %r", e)
            await interaction.response.send_message("Failed to save reason.", ephemeral=True)


class MutedActionView(discord.ui.View):
    """View containing a Select for muted members and action buttons.

    - The Select chooses the member (stored in `self.selected_member_id`).
    - Buttons operate on the selected member. Buttons will prompt helpful
      messages if no member is selected or the clicker isn't the original
      command invoker.
    """
    ...
    def __init__(self, cog: "MuteList", ctx: commands.Context, members: list[discord.Member]):
        ...
        if options:
            self.add_item(discord.ui.Select(placeholder="Select a muted member...", min_values=1, max_values=1, options=options, custom_id="mutelist_select"))

    async def interaction_check(self, interaction: discord.Interaction) -> bool:  # restrict who can use
        # allow only the original invoker to operate the menu
        if interaction.user.id != self.invoker.id:
            await interaction.response.send_message("Only the command invoker may use this menu.", ephemeral=True)
            return False
        return True

    @discord.ui.button(label="Unmute", style=discord.ButtonStyle.green, custom_id="mutelist_unmute")
    async def unmute(self, button: discord.ui.Button, interaction: discord.Interaction) -> None:
        if not self.selected_member_id:
            await interaction.response.send_message("Select a member first from the dropdown.", ephemeral=True)
            return
        ...
        if not to_remove:
            await interaction.followup.send(f"{member} has no configured mute roles.", ephemeral=False)
            return
        try:
            await member.remove_roles(*to_remove, reason=f"Unmuted by {interaction.user}")
            await self.cog.clear_mute(guild, member.id)
            await interaction.followup.send(f"Removed mute roles from {member}.", ephemeral=False)
        except discord.Forbidden:
            await interaction.followup.send("I don't have permission to remove roles.", ephemeral=True)
        except Exception as e:
            log.exception("Unmute failed: %r", e)
            await interaction.followup.send("Failed to unmute.", ephemeral=True)

    @discord.ui.button(label="Kick", style=discord.ButtonStyle.blurple, custom_id="mutelist_kick")
    async def kick(self, button: discord.ui.Button, interaction: discord.Interaction) -> None:
        ...
        try:
            ...
            await self.cog.clear_mute(guild, member.id)
            await interaction.followup.send(f"Kicked {member}.", ephemeral=False)
        except discord.Forbidden:
            await interaction.followup.send("I don't have permission to kick that member.", ephemeral=True)
        except Exception as e:
            log.exception("Kick failed: %r", e)
            await interaction.followup.send("Failed to kick.", ephemeral=True)

    @discord.ui.button(label="Ban", style=discord.ButtonStyle.danger, custom_id="mutelist_ban")
    async def ban(self, button: discord.ui.Button, interaction: discord.Interaction) -> None:
        ...
        try:
            ...
            await self.cog.clear_mute(guild, member.id)
            await interaction.followup.send(f"Banned {member}.", ephemeral=False)
        except discord.Forbidden:
            await interaction.followup.send("I don't have permission to ban that member.", ephemeral=True)
        except Exception as e:
            log.exception("Ban failed: %r", e)
            await interaction.followup.send("Failed to ban.", ephemeral=True)

    @discord.ui.button(label="Set Reason", style=discord.ButtonStyle.secondary, custom_id="mutelist_setreason")
    async def setreason(self, button: discord.ui.Button, interaction: discord.Interaction) -> None:
        ...
        if guild is None:
            await interaction.response.send_message("Guild context missing.", ephemeral=True)
            return
        modal = MemberReasonModal(self.cog, guild, self.selected_member_id, interaction.user)
        try:
    ...
    @discord.ui.button(label="Cancel", style=discord.ButtonStyle.secondary, custom_id="mutelist_cancel")
    async def cancel(self, button: discord.ui.Button, interaction: discord.Interaction) -> None:
        try:
            await interaction.response.send_message("Menu closed.", ephemeral=False)
            # disable the view so buttons stop working
            self.stop()
            try:
    ...
    async def on_timeout(self) -> None:
        # disable interaction when timed out
...
class MuteList(commands.Cog):
    ...
    async def mutedlist(self, ctx: commands.Context):
        ...
        The resulting message includes a dropdown to pick a member and
        buttons to perform actions. The list shows names and IDs.
        """
        guild = ctx.guild
        ...
        # build lines with visible name and id
        lines: list[str] = []
